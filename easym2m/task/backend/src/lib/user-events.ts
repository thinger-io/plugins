...
export class UserEvents extends EventEmitter {
  ...
  getRecent(filters?: EventFilters): UserEvent[] {
    ...
    if (filters) {
      ...
      if (filters.application) {
        filtered = filtered.filter(e => e.application === filters.application);
      }
    }
  ...
  /**
   * Gets all events for a specific device
   *
   * @param deviceEui Device EUI
   * @param limit Optional limit
   * @returns Array of events for the device
   */
  getByDevice(deviceEui: string, limit?: number): UserEvent[] {
    return this.getRecent({ device: deviceEui, limit });
  }

  /**
   * Gets all events for a specific application
   *
   * @param applicationId Application ID
   * @param limit Optional limit
   * @returns Array of events for the application
   */
  getByApplication(applicationId: string, limit?: number): UserEvent[] {
    return this.getRecent({ application: applicationId, limit });
  }
